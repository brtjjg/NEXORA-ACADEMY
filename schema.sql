...
CREATE TABLE student_profiles (
    ...
    admission_status    TEXT NOT NULL DEFAULT 'pending_payment'
                        ...
                        ('pending_payment','paid_pending_verification',
                         'approved','rejected')),
    ...
    national_id_image   TEXT,                    -- storage path, not base64
    updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- Sessions (if you use server-side sessions instead of JWT)
CREATE TABLE sessions (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id         UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    token_hash      TEXT NOT NULL UNIQUE,        -- hash of the session token
    ip_address      INET,
    user_agent      TEXT,
...
-- ============================================================
-- 2. APPLICATIONS (Admission pipeline)
-- ============================================================

CREATE TABLE applications (
    id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    application_id      TEXT UNIQUE NOT NULL,    -- e.g., NXA-APP-2026-00001
    user_id             UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    full_name           TEXT NOT NULL,
    ...
    status              TEXT NOT NULL DEFAULT 'payment_due'
                        CHECK (status IN
                        ('payment_due','paid','approved','rejected')),
...
CREATE TABLE application_history (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    application_id  UUID NOT NULL REFERENCES applications(id) ON DELETE CASCADE,
    action          TEXT NOT NULL,               -- created, paid, approved, rejected
    note            TEXT,
...
-- Admission number counter (per year)
...
CREATE TABLE courses (
    ...
    description     TEXT,
    instructor_name TEXT,                        -- free text since no instructor table
    duration        TEXT,
...
CREATE INDEX idx_course_discounts_course ON course_discounts(course_id);
...
CREATE UNIQUE INDEX uq_modules_position ON modules(course_id, position);
...
CREATE UNIQUE INDEX uq_lessons_position ON lessons(module_id, position);

-- Lesson attachments (PDFs, images)
...
CREATE TABLE transactions (
    id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    transaction_id      TEXT UNIQUE NOT NULL,     -- NXA-TXN-2026-00001
...
-- Wallet ledger (mirror of transactions for user-facing wallet)
CREATE TABLE wallet_transactions (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    wallet_tx_id    TEXT UNIQUE NOT NULL,         -- NXA-WLT-2026-00001
    user_id         UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE TABLE certificates (
    id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    certificate_id      TEXT UNIQUE NOT NULL,     -- NXA-CP-2026-000001
...
CREATE TABLE sponsorships (
    ...
    covered_courses UUID[],                       -- array of course IDs
...
CREATE TABLE activities (
    ...
    activity_type   TEXT NOT NULL,               -- account, enrollment, admission
    title           TEXT NOT NULL,
    description     TEXT,
    metadata        JSONB,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
...
CREATE TRIGGER trg_course_discounts_updated
    BEFORE UPDATE ON course_discounts
    FOR EACH ROW EXECUTE FUNCTION set_updated_at();
